...
@bot.on(sudo_cmd(pattern="spam|fspam (.*)", allow_sudo=True))
async def spammer(e):
...
@bot.on(sudo_cmd(pattern="bigspam|mcspam", allow_sudo=True))
...
@bot.on(sudo_cmd(pattern="dspam|opspam (.*)", allow_sudo=True))
async def spammer(e):
    if e.fwd_from:
        return
